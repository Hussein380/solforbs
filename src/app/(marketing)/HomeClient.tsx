...
import {
  GraduationCap, Hotel, Building2, Tractor,
  Stethoscope, ShoppingBag, Factory, Landmark,
  CalendarClock, WalletCards, MessagesSquare,
} from "lucide-react";
...
/* Industry-specific icon color classes */
const industryIconClass: Record<string, string> = {
  Education:     "icon-wrap-blue",
  Hospitality:   "icon-wrap-green",
  "Real Estate": "icon-wrap-indigo",
  Agriculture:   "icon-wrap-amber",
  Healthcare:    "icon-wrap-rose",
  Retail:        "icon-wrap-teal",
  Manufacturing: "icon-wrap-purple",
  Government:    "icon-wrap-slate",
};
...
/* ── Interactive Features (Product Capabilities) ─────────────────── */
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1.4fr", gap: 48, alignItems: "center", marginTop: 40 }} className="stack-mobile">
            ...
            <div key={cap.id} onClick={() => setActive(i)}
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: isActive ? 12 : 0, transition: "margin 0.3s ease" }}>
                <div style={{
                  ...
                  background: isActive ? "var(--gradient-brand)" : "rgba(0,0,0,0.04)",
                  color: isActive ? "#fff" : "#6B7280",
                  transition: "all 0.3s ease",
                }}>
                  <Icon size={22} strokeWidth={isActive ? 2.5 : 2} />
                </div>
                <h3 style={{ fontSize: 17, fontWeight: 600, color: isActive ? "var(--text-primary)" : "var(--text-secondary)" }}>
                  {cap.title}
                </h3>
              </div>
              <motion.div initial={false} animate={{ height: isActive ? "auto" : 0, opacity: isActive ? 1 : 0 }} style={{ overflow: "hidden" }}>
                <p style={{ fontSize: 14, color: "var(--text-secondary)", lineHeight: 1.6, paddingLeft: 60 }}>{cap.desc}</p>
              </motion.div>
            </div>
          );
        })}
      ...
      <div style={{ position: "relative", width: "100%", aspectRatio: "4/3", background: "#fff",
        borderRadius: 24, border: "1px solid rgba(0,0,0,0.06)", boxShadow: "0 24px 60px rgba(0,0,0,0.05)", overflow: "hidden" }}>
        {/* Fake browser chrome */}
        <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 48, background: "#F7F9FC",
          borderBottom: "1px solid rgba(0,0,0,0.04)", display: "flex", alignItems: "center", padding: "0 20px", gap: 6 }}>
        ...
        <div style={{ padding: 32, paddingTop: 48, display: "flex", alignItems: "center", justifyContent: "center", height: "100%" }}>
          <motion.div key={active} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.4 }}>
            <div style={{ width: 120, height: 120, borderRadius: "50%", background: "var(--brand-tint-8)",
              display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto", marginBottom: 24 }}>
              {(() => { const ActiveIcon = capabilities[active].icon; return <ActiveIcon size={48} color="var(--brand-mid)" strokeWidth={1.5} />; })()}
            </div>
            <p style={{ textAlign: "center", fontWeight: 600, color: "var(--text-primary)", fontSize: 18 }}>System Module Active</p>
            <p style={{ textAlign: "center", color: "var(--text-secondary)", fontSize: 13, marginTop: 8 }}>
              {capabilities[active].title} module is ready for configuration.
            </p>
...
export default function HomeClient({ projects = [] }: { projects?: any[] }) {
  ...
  return (
    <>
      ...
      <section ref={heroRef} className="stack-mobile hero-section" style={{
        ...
        <motion.div className="section-pad hero-content-wrap" style={{ position: "relative", zIndex: 4, maxWidth: 1100, margin: "0 auto", width: "100%", opacity: heroOpacity }}>
          ...
          {(() => {
            ...
            const rawUrl = featuredProject?.subdomain ? `https://${featuredProject.subdomain}` : featuredProject?.liveUrl;
            const isSafeExternal = rawUrl && rawUrl !== "#" && rawUrl !== "https://#" && rawUrl !== "http://#" && rawUrl.startsWith("http");
            
            return (
              <a href={isSafeExternal ? rawUrl : "/#products"} target={isSafeExternal ? "_blank" : undefined} rel={isSafeExternal ? "noopener noreferrer" : undefined} style={{ display: "inline-block", textDecoration: "none" }}>
                <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}
                  style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 24, padding: "4px 12px", marginLeft: "-12px",
                    borderRadius: 20, transition: "background 0.2s" }}
                  onMouseEnter={e => (e.currentTarget.style.background = "#F3F4F6")}
                  onMouseLeave={e => (e.currentTarget.style.background = "transparent")}>
                  <StatusBadge status="live" />
                  <span style={{ fontSize: 13, color: "var(--text-secondary)", fontWeight: 500 }}>{bannerText}</span>
          ...
          <div className="w-full-mobile text-center-mobile" style={{ maxWidth: "52%", minWidth: 280 }}>
            <h1 style={{ fontSize: "clamp(32px, 5.5vw, 68px)", fontWeight: 700, marginBottom: 20, color: "var(--text-primary)" }}>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.7, delay: 0.9 }}
            ...
            {stats.map(s => (
              <div key={s.label}>
                <div style={{ fontSize: "clamp(24px, 2.8vw, 36px)", fontWeight: 700, fontFamily: "'Space Grotesk', sans-serif" }}
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.8 }}
          ...
          <span style={{ fontSize: 10, letterSpacing: "0.14em", textTransform: "uppercase", color: "var(--text-tertiary)" }}>Scroll</span>
          <motion.div animate={{ y: [0, 5, 0] }} transition={{ duration: 1.4, repeat: Infinity, ease: "easeInOut" }}
            style={{ width: 1, height: 28, background: "linear-gradient(to bottom, var(--brand-sky), transparent)" }} />
        </motion.div>
      </section>


      {/* ═══ PRODUCTS ═══════════════════════════════════════════════ */}
      <section id="products" className="section-pad-sm" style={{ background: "var(--bg-alt)", borderBottom: "1px solid rgba(0,0,0,0.06)", overflow: "hidden" }}>
        <div style={{ maxWidth: 1300, margin: "0 auto", paddingBottom: 60, paddingLeft: 16, paddingRight: 16 }}>
          <Reveal>
        ...
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 16 }}>
            {industries.map((ind, i) => {
              const Icon = ind.icon;
              const iconClass = industryIconClass[ind.name] ?? "icon-wrap-slate";
              const tagClass = ind.status === "live" ? "tag-live" : ind.status === "in_development" ? "tag-dev" : "tag-planned";
              return (
                <Reveal key={ind.name} delay={i * 0.04}>
                  <motion.div whileHover={{ y: -4, boxShadow: "0 12px 30px rgba(0,0,0,0.07)" }}
                    style={{ padding: 24, borderRadius: 16, background: "#fff", border: "1px solid rgba(0,0,0,0.06)",
                      transition: "box-shadow 0.3s, border-color 0.3s", height: "100%", display: "flex", flexDirection: "column" }}>
                    <div className={`icon-wrap ${iconClass}`}><Icon size={22} strokeWidth={2} /></div>
                    <h3 style={{ fontSize: 17, fontWeight: 700, marginBottom: 8, color: "var(--text-primary)" }}>{ind.name}</h3>
                    <p style={{ fontSize: 14, color: "var(--text-secondary)", lineHeight: 1.6, flex: 1 }}>{ind.desc}</p>
                  </motion.div>
                </Reveal>
              );
            })}
      ...
      {/* ═══ WHY SOLFORBS ════════════════════════════════════════════ */}
      <section className="section-pad-sm" style={{ background: "var(--bg-alt)", borderBottom: "1px solid rgba(0,0,0,0.06)" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <Reveal>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: 36 }}>
            {whyPoints.map((pt, i) => (
              <Reveal key={pt.num} delay={i * 0.07}>
                <div style={{ paddingLeft: 18, borderLeft: "2px solid var(--border-brand)" }}>
                  <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.1em", color: "var(--brand-mid)", display: "block", marginBottom: 10 }}>
                    {pt.num}
                  </span>
                  <h3 style={{ fontSize: 15, fontWeight: 600, marginBottom: 8, color: "var(--text-primary)" }}>{pt.title}</h3>
                  <p style={{ fontSize: 13, color: "var(--text-secondary)", lineHeight: 1.7 }}>{pt.desc}</p>
                </div>
              </Reveal>
            ))}
          </div>
      ...
      <section className="section-pad-sm stack-mobile" style={{ background: "#fff", borderBottom: "1px solid rgba(0,0,0,0.06)" }}>
        <div style={{ maxWidth: 760, margin: "0 auto" }}>
          <Reveal>
            ...
            <h2 className="section-title" style={{ marginBottom: 14 }}>How we work with you</h2>
            <p className="section-subtitle" style={{ maxWidth: 520, marginBottom: 56 }}>
          ...
          <div ref={roadmapRef} style={{ position: "relative", paddingLeft: 96 }}>
            <div style={{ position: "absolute", left: 70, top: 0, bottom: 0, width: 1, background: "rgba(0,0,0,0.08)" }}>
              <motion.div style={{ position: "absolute", top: 0, left: 0, right: 0,
                background: "var(--gradient-brand)", height: lineH }} />
            </div>

            {implementationSteps.map((item, i) => (
              <Reveal key={i} delay={i * 0.07}>
                <div style={{ display: "flex", alignItems: "flex-start", paddingBottom: 32, position: "relative" }}>
                  <span style={{ position: "absolute", left: -96, fontSize: 11, fontWeight: 700, color: "var(--text-tertiary)",
                    fontVariantNumeric: "tabular-nums", paddingTop: 3, width: 56, textAlign: "right", letterSpacing: "0.05em" }}>
                    {item.step.toUpperCase()}
                  </span>
                  <div style={{ position: "absolute", left: -25, top: 5, width: 8, height: 8, borderRadius: "50%",
                    background: "var(--brand-mid)", boxShadow: "0 0 0 3px var(--brand-tint-20)", border: `2px solid var(--brand-mid)` }} />
                  <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                    <span style={{ fontSize: 15, fontWeight: 700, color: "var(--text-primary)" }}>{item.item}</span>
                    <span style={{ fontSize: 14, color: "var(--text-secondary)", lineHeight: 1.5, paddingRight: 20 }}>{item.desc}</span>
                  </div>
                </div>
              </Reveal>
            ))}
          ...
          <Reveal delay={0.3}>
            <Link href="/contact" className="footer-link" style={{ fontSize: 14, fontWeight: 500, display: "inline-block", marginTop: 10 }}>
              Book a consultation →
            </Link>
          </Reveal>
        </div>
      ...
      {/* ═══ CTA ═════════════════════════════════════════════════════ */}
      <section className="section-pad-sm stack-mobile" style={{ position: "relative", textAlign: "center", overflow: "hidden",
        background: "var(--gradient-brand)" }}>
        {/* Dot grid */}
        <div style={{ position: "absolute", inset: 0, backgroundImage: "radial-gradient(rgba(255,255,255,0.12) 1px, transparent 1px)",
          backgroundSize: "24px 24px", pointerEvents: "none" }} />
        <div style={{ position: "relative", zIndex: 2 }}>
          <Reveal>
            <p style={{ fontSize: 12, fontWeight: 700, letterSpacing: "0.14em", textTransform: "uppercase",
              color: "rgba(255,255,255,0.65)", marginBottom: 16 }}>Get started</p>
            <h2 style={{ fontSize: "clamp(30px, 5vw, 56px)", fontWeight: 700, letterSpacing: "-0.03em", lineHeight: 1.1,
              color: "#fff", marginBottom: 16 }}>
              Ready to transform<br />your organization?
            </h2>
            <p style={{ color: "rgba(255,255,255,0.72)", marginBottom: 36, fontSize: 16, maxWidth: 400, margin: "0 auto 36px" }}>
              Talk to us about what you need. We&apos;ll get back within one business day.
            </p>
            <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap" }}>
              <Link href="/contact" className="btn btn-white">Book a demo</Link>

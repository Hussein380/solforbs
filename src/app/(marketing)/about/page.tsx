...
export default function AboutPage() {
  return (
    <>
      ...
      <section className="section-pad-sm" style={{ background: "var(--bg-alt)", borderBottom: "1px solid rgba(0,0,0,0.04)" }}>
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          <Reveal>
            <span className="section-label">What we do</span>
            <h2 className="section-title" style={{ marginBottom: 24 }}>Software products. Not services.</h2>
      ...
      <section className="section-pad-sm" style={{ background: "#fff", borderBottom: "1px solid rgba(0,0,0,0.04)" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <Reveal>
            ...
            <h2 className="section-title" style={{ marginBottom: 56 }}>What we stand for</h2>
          </Reveal>

          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 48 }}>
            {values.map((val, i) => (
              <Reveal key={val.title} delay={i * 0.1}>
                <div style={{ paddingLeft: 24, borderLeft: "3px solid var(--border-brand)", position: "relative" }}>
                  <div style={{ position: "absolute", left: -14, top: 0, width: 24, height: 24, borderRadius: "50%", background: "#fff", border: "4px solid var(--border-brand)" }} />
                  <span style={{ fontSize: 13, fontWeight: 700, letterSpacing: "0.1em", color: "var(--brand-mid)", display: "block", marginBottom: 12 }}>
                    0{i + 1}
                  </span>
                  <h3 style={{ fontSize: 20, fontWeight: 700, marginBottom: 12, color: "var(--text-primary)", fontFamily: "'Space Grotesk', sans-serif" }}>{val.title}</h3>
                  <p style={{ fontSize: 16, color: "var(--text-secondary)", lineHeight: 1.6 }}>{val.desc}</p>
                </div>
              </Reveal>
            ))}
          </div>
      ...
      <section className="section-pad-sm" style={{ background: "var(--bg-alt)", textAlign: "center", borderTop: "1px solid rgba(0,0,0,0.04)" }}>
        <Reveal>
          <h2 className="section-title" style={{ marginBottom: 16 }}>Want to know more?</h2>
          <p style={{ color: "var(--text-secondary)", marginBottom: 40, fontSize: 18 }}>
            Reach out — we&apos;re happy to talk about what we&apos;re building and why.
          </p>
